...
// Parse ONE stated value fragment ("≈ 34.6 N", "x = 7", "40,000 N·s", "3/4",
// "3.0 × 10^8 m/s") into a number/Unit, or null when it isn't ONE unambiguous numeric
// value. The text is learner/model-controlled, so it runs through the same sandbox as
// the check. `allowPercent` (off by default for parseAnswer's single-value contract):
// when on, a bare "25%" yields BOTH legitimate readings {25, 0.25} via parseValues.
function parseOneValue(text, unitsHint = "") {
...
// Parse a stated answer into ONE value, or null when it isn't ONE unambiguous numeric
// value (multi-root / percent / multiple numbers all return null). The single-value
// contract used by checkSolve and the public parseAnswer.
export function parseAnswer(text, unitsHint = "") {
  return parseOneValue(text, unitsHint);
}

// Strip ONE bare trailing/leading percent token to its number, returning BOTH legitimate
// readings of "N%": the percent number itself (N, e.g. "what % …" → 25) and the fraction
// (N/100, e.g. a probability/efficiency 0.25). Returns [] when the fragment isn't a clean
// bare percentage. Matching the verified value against EITHER is raise-only-safe: both are
// valid interpretations of "N%", so a match means the learner's magnitude is correct under
// a percent reading; a non-match never lowers anything (applyLearnerCorrection is raise-only).
function percentReadings(text) {
  if (typeof text !== "string") return [];
  let s = text.slice(0, 300).trim();
  s = s.replace(/^[A-Za-z][A-Za-z0-9_]{0,20}\s*[=≈~:]\s*/, ""); // "p = 25%" → "25%"
  s = s.replace(/^[≈~]\s*/, "").replace(/^(approximately|approx\.?|about|roughly)\s+/i, "");
  s = s.replace(/[.,;]\s*$/, "").trim();
  const m = normalizeMathText(s).match(/^(-?\d+(?:\.\d+)?(?:[eE][+-]?\d+)?)\s*%$/);
  if (!m) return [];
  const n = Number(m[1]);
  return Number.isFinite(n) ? [n, n / 100] : [];
}

/**
 * FIX 5: parse a stated answer into the SET of candidate numeric values it could mean —
 * so the raise-only correction can match MULTI-ROOT answers ("x = 3 or x = -2", "3, -2":
 * verify each root) and bare PERCENTAGES ("25%" → {25, 0.25}). Always returns an array
 * (possibly empty); a single clean value yields a one-element array. Each candidate is a
 * number/Unit that passed the same sandbox as the check — nothing un-evaluated leaks
 * through. Used ONLY by applyLearnerCorrection (never to LOWER a grade), so a stray extra
 * candidate can never mis-verdict a correct learner: a wrong root simply won't match.
 */
export function parseValues(text, unitsHint = "") {
  if (typeof text !== "string" || !text.trim()) return [];
  const single = parseOneValue(text, unitsHint);
  if (single != null) return [single];
  // Multi-root / list: split on "or"/"and"/commas/semicolons and parse each piece.
  const pieces = text
    .slice(0, 300)
  ...
  if (pieces.length > 1) {
    for (const p of pieces) {
      const v = parseOneValue(p, unitsHint);
      if (v != null) out.push(v);
    }
  }
  // Bare percentage (the single-value parser skips "%"): add both readings.
  for (const v of percentReadings(text)) out.push(v);
  return out;
}
...
export function applyLearnerCorrection({ value, learnerAnswer, finalAnswerMatches, rubric, errors }) {
  ...
  // FIX 5: the learner may state MULTIPLE roots ("x = 3 or x = -2") or a percentage
  // ("25%"). parseValues yields every candidate value; the answer "matches" when the
  // machine-verified value equals ANY candidate (a correct root / a valid percent
  // reading is present), is "false" only when there are comparable candidates and NONE
  // match, and "no signal" (return early, fail open) when nothing is comparable.
  const candidates = parseValues(learnerAnswer);
  if (candidates.length === 0) return out;
  let matches = null; // null = no comparable candidate yet (fail open)
  for (const c of candidates) {
    const eq = numericallyEqual(c, value);
    if (eq === true) { matches = true; break; }
    if (eq === false) matches = false; // comparable but unequal — a real mismatch signal
  }
