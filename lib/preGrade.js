...
export function preGradeDock(reasoning) {
  ...
  if (trimmed === "" || norm === "") {
    ...
    // Ultra-short BARE fragment with no real reasoning: a lone number or letter ("1", "5",
    // "42", "x"). Previously a single digit slipped past the too-short gate's HAS_DIGIT
    // escape, sending trivial spam to the paid grader and removing the deterministic low
    // floor (audit 05 P1-3). Scoped tightly so it never catches a genuine result
    // ("100000000", "x = 1111111" are >3 non-space chars), a 3-char chemical formula
    // ("h2o"/"co2" survive the length-3 word test), or a structured fragment carrying a
    // math operator ("x=5", "2+2=4" — those are minimal but real and stay graded by design).
    norm.replace(/\s/g, "").length <= 3 &&
    !HAS_MATH_OP.test(raw) &&
    norm.split(" ").filter((w) => w.length >= 3).length === 0
  ) {
    reason = "too short / trivial";
  ...
  return dockVerdict(reason);
}

// The complete, grader-shaped near-zero verdict a route can return directly: the
// DOCK_SCORE outcome + an all-zero rubric + generic, answer-free coaching. Shared by
// preGradeDock (empty/idk/gibberish) and the injection auto-dock (audit P2-3) so both
// produce an identical learner-facing result — a docked attempt that drives the rating
// DOWN and reveals NO worked solution.
function dockVerdict(reason) {
  return {
    docked: true,
    reason,
...
// Forced near-zero dock for a HIGH-confidence prompt injection (audit P2-3). The
// routes call this AFTER scanForInjection flags a strong instruction-override /
// role-injection attempt: a successful injection could otherwise inflate the rubric
// to all-4s, so we skip the LLM grade entirely and return the same near-zero verdict
// as an empty/"idk" answer. The event is still logged (reportInjection) for review.
export function injectionDock() {
  return dockVerdict("prompt-injection");
}
