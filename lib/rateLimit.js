...
// Honor any finite NON-NEGATIVE override, including an explicit 0 (audit 03 P2-11): a 0
// budget is a deliberate emergency kill-switch, not a misconfiguration to silently ignore.
// Only an unset/empty/non-numeric/negative value falls back to the default.
const envBudget = (name, dflt) => {
  const v = Number(process.env[name]);
  return Number.isFinite(v) && v >= 0 ? v : dflt;
};

// SPLIT BUDGET POOLS (red-team Finding 3). The global Groq budget used to be a SINGLE
// shared window, so an IP-rotating flood on the UNAUTHENTICATED routes (/api/generate,
// /api/grade, guest diagnostics) could saturate it and 429 EVERY caller — including
// signed-in / paying users (a cost-DoS converted into an availability-DoS, T1499.003).
// Splitting into two independent pools isolates that blast: a guest flood can exhaust the
// guest pool without touching the authenticated pool. The GUEST pool keeps the original
// bucket names ("global:groq" / "global:img") so existing behavior/budgets are unchanged;
// the AUTH pool ("global:groq:auth" / "global:img:auth") is reachable only after a VERIFIED
// JWT (routes pass pool:"auth" only for an authenticated caller), so a guest can't opt into
// it by spoofing a header. Each pool's budget is independently env-tunable.
function poolBuckets(pool) {
  const suffix = pool === "auth" ? ":auth" : "";
  return { groq: `global:groq${suffix}`, img: `global:img${suffix}` };
}
function poolBudgets(pool) {
  if (pool === "auth") {
    return {
      maxGroq: envBudget("GLOBAL_GROQ_BUDGET_AUTH_PER_MIN", 600),
      maxImg: envBudget("GLOBAL_IMG_BUDGET_AUTH_PER_MIN", 120),
    };
  }
  return {
    maxGroq: envBudget("GLOBAL_GROQ_BUDGET_PER_MIN", 300),
    maxImg: envBudget("GLOBAL_IMG_BUDGET_PER_MIN", 60),
  };
}
 ...
 * checkRateLimit. `pool` selects the isolated budget window (Finding 3):
 * "guest" (default) → "global:groq"/"global:img"; "auth" → "global:groq:auth"/
 * "global:img:auth". A caller MUST only pass pool:"auth" for a JWT-verified user.
 */
export async function chargeGlobalGroq(n = 1, { img = 0, pool = "guest" } = {}) {
  const { maxGroq, maxImg } = poolBudgets(pool);
  const buckets = poolBuckets(pool);
  ...
  for (let i = 0; i < want; i++) {
    res = await checkRateLimit(buckets.groq, { max: maxGroq, failClosed: true });
    if (!res.ok) {
      await refundGlobalGroq(chargedGroq, { img: chargedImg, pool });
      return res;
    }
    chargedGroq += 1;
  }
  for (let i = 0; i < img; i++) {
    res = await checkRateLimit(buckets.img, { max: maxImg, failClosed: true });
    if (!res.ok) {
      await refundGlobalGroq(chargedGroq, { img: chargedImg, pool });
      return res;
    }
 ...
 */
export async function refundGlobalGroq(n = 1, { img = 0, pool = "guest" } = {}) {
  const groqN = Math.max(0, Math.floor(n));
  const imgN = Math.max(0, Math.floor(img));
  const buckets = poolBuckets(pool); // Finding 3: refund the SAME pool that was charged
  await refundRateLimit(buckets.groq, groqN);
  if (imgN > 0) await refundRateLimit(buckets.img, imgN);
}
...
// Exported so a route can give back a slot it charged before deciding the request didn't
// count (e.g. the free daily-practice cap on a duplicate or a docked non-attempt — audit 02 P1-4).
export async function refundRateLimit(bucket, n = 1) {
  if (n <= 0) return;
  const sb = rlClient();
  if (sb) {
    try {
