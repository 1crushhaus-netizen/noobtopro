...
//
// The Learn library turns the in-app concept hub — the full set of curated guides
// across math / physics / chemistry and four ranks — into fully server-rendered,
...
// Accessible (AA-contrast) per-subject color for TEXT. The bright subjectColor()
// accents are only ~3.8–4:1 on white and miss WCAG AA at small sizes, so small
// labels (e.g. the Learn-page eyebrows) must use these theme-aware --*-text tokens
// instead (≥4.5:1 in both light and dark). Keep subjectColor() for borders/graphics.
const SUBJECT_TEXT_VAR = {
  math: "var(--math-text)",
  physics: "var(--phys-text)",
  chemistry: "var(--chem-text)",
};
export function subjectTextColor(subject) {
  return SUBJECT_TEXT_VAR[subject] || "var(--muted)";
}
...
// SERP <title> for a concept page: "<label> · <Subject>". Deliberately compact —
// the brand suffix and the rank parenthetical are dropped so the title stays under
// the ~60-char / ~580px limit Google truncates at (the old "<label> · <Subject>
// (<Rank>) — noobtopro" form ran 157/224 concepts over length). The subject keyword
// is kept (it carries real search intent); rank still lives in the URL, breadcrumb,
// H1 eyebrow, and the branded og:title. Pages set this via `title: { absolute }` so
// the root template does NOT re-append " — noobtopro".
export function conceptTitle(subject, label) {
  return `${label} · ${subjectLabel(subject)}`;
}
...
// Next REPLACES (not merges) the parent openGraph when a child sets one, so
// siteName/locale must be re-stated here or every Learn page loses the root
