...
function extractJSON(text) {
  ...
  // Fast path: the de-fenced text is already valid JSON. Try this before the
  // first-brace/last-brace slice, which can grab the wrong "}" when a string
  // value legitimately contains braces (e.g. "use the set {a, b}").
  try {
    return JSON.parse(t);
  } catch {
    /* fall through to brace extraction for prose-wrapped JSON */
  }
  const s = t.indexOf("{");
