...
FEEDBACK — the learner has ALREADY SUBMITTED, so be concretely instructive (this is shown AFTER grading):
- strengths: 1–3 SPECIFIC things this answer did well, citing what they actually did. [] (empty) if there was nothing substantive.
- improvements: 1–3 SPECIFIC, actionable things they would need to do to raise their score toward 100 — each aimed at a WEAK rubric dimension and describing the REASONING to add (e.g. "state why you add the quantities rather than multiply", "show the intermediate sum before the total"), NOT vague advice like "be clearer".
- workedSolution: the FULL step-by-step worked solution to THIS question, ending with the final answer. The learner has already attempted it, so revealing the complete solution here is INTENDED — it is how they learn. Be clear, correct, and complete; show the reasoning a top (100-scoring) answer would.
- correctnessNote: <=25 words on whether their conclusion holds.
- socraticHint: ONE guiding question pointing at the next step they missed.
- microLesson: <=90 words teaching the underlying CONCEPT they are weak on — the principle in general.

Respond with ONLY valid JSON, no markdown, with "rationale" FIRST so you reason before scoring:
{"rationale":"<1-2 sentences: what the reasoning showed, justifying the scores below>","rubric":{"conceptual_understanding":<0-4>,"logical_structure":<0-4>,"strategy":<0-4>,"execution_accuracy":<0-4>,"communication":<0-4>},"reasoningScore":<int 0-100 consistent with the rubric>,"strengths":["<specific thing done well>","..."],"improvements":["<specific, actionable step to raise the score>","..."],"workedSolution":"<the full step-by-step solution + final answer (post-grade reveal)>","correctnessNote":"...","socraticHint":"...","microLesson":"...","weakConcepts":["..."],"newScoreSuggestion":<int 0-100 updated subject level>}`;

export const LEARN_SYS = `You are the concept tutor for noobtopro. The learner is weak on a specific CONCEPT in mathematics, physics, or chemistry. TEACH the concept in depth: build genuine intuition AND explain the underlying mechanism — including the PROOF, DERIVATION, or the reason WHY it is true — clearly and rigorously.
