...
Respond with ONLY valid JSON, no markdown:
{"overview":"<2-4 sentence intuition: what this concept is and why it matters>","keyIdeas":["<core principle to internalize>","<another>","..."],"socraticQuestions":["<question that prompts the learner's own thinking>","..."],"pitfalls":["<common mistake or misconception>","..."],"tryThis":"<a concrete way to practise this concept or a general approach to problems involving it — a nudge, NOT an answer>"}`;
 ...
 */
export async function groqJSON({ system, user, image, maxTokens }) {
  ...
  // A retry only helps a PARSE failure or an HTTP 400 (model rejected
  // response_format). A hard upstream error (401/403/429/5xx) won't be fixed by
  // retrying and would just double the Groq cost / 429 pressure — so re-throw it.
  try {
    return extractJSON(await rawCall({ model, system, userContent, jsonMode: true, maxTokens }));
  } catch (e1) {
    if (e1 && typeof e1.status === "number" && e1.status !== 400) {
      // Hard upstream error on the first model. For an image grade, a 429/5xx is
      // specific to the VISION model — TEXT_MODEL is a separate endpoint, so a
      // degraded text-only grade is still a valid recovery. Only 401/403 (the
      // shared API key) make any further call pointless.
      if (image && e1.status !== 401 && e1.status !== 403) {
    ...
    try {
      ...
      // Vision call failed for a retryable reason → fall back to grading text-only.
      if (image && (!e2 || typeof e2.status !== "number" || e2.status === 400)) {
        return extractJSON(await rawCall({ model: TEXT_MODEL, system, userContent: user, jsonMode: false, maxTokens }));
      }
      throw e2;
    }
