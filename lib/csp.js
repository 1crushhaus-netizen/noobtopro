...
export function buildContentSecurityPolicy(nonce) {
  ...
  const scriptSrc = nonce
    ? `script-src 'self' 'nonce-${nonce}' https://va.vercel-scripts.com https://analytics.ahrefs.com`
    : "script-src 'self' 'unsafe-inline' https://va.vercel-scripts.com https://analytics.ahrefs.com";
  ...
  return [
    ...
    scriptSrc,
    // analytics.ahrefs.com is allow-listed in BOTH script-src (to load analytics.js)
    // and connect-src (the cookieless Ahrefs Web Analytics beacon posts pageviews back
    // to the same host). Keep the two in sync.
    `connect-src 'self' ${supabaseConnectSrc()} https://vitals.vercel-insights.com https://analytics.ahrefs.com`,
  ].join("; ");
}
