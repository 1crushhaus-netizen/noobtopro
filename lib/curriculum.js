...
// token set for the legacy free-text fallback.
const RESOLVE_INDEX = {};
for (const subject of Object.keys(CURRICULUM)) {
  ...
  for (const rank of RANKS) {
    for (const c of conceptsFor(subject, rank)) {
      ...
      concepts.push({ key: c.key, tokens: new Set(norm.split(" ").filter(Boolean)) });
    }
  }
  RESOLVE_INDEX[subject] = { keySet, labelIndex, concepts };
}
...
export function resolveConceptKey(subject, value) {
  ...
  // 3) legacy free text: the most specific concept whose label tokens are all
  // present in the phrase (require >=2 shared label tokens so a lone stopword
  // like "of"/"and" can't match). Longest fully-covered label wins.
  const phraseTokens = new Set(norm.split(" ").filter(Boolean));
  let best = null;
  for (const c of idx.concepts) {
    if (c.tokens.size < 2) continue;
    let covered = true;
    for (const t of c.tokens) {
      if (!phraseTokens.has(t)) { covered = false; break; }
    }
    if (covered && (!best || c.tokens.size > best.tokens.size)) best = c;
  }
