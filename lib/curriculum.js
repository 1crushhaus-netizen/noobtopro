...
// Look up a single concept's metadata ({ key, label, strand, rank }) by its key
// within a subject — the server-side ALLOWLIST for the concept-practice drill
// (increment 3): a request's conceptKey is honored only if this returns a hit, so a
// drill can target only a real curriculum concept. Prototype-safe (own-property
// scan via conceptsFor). Returns null for an unknown subject/key.
export function conceptByKey(subject, key) {
  if (typeof subject !== "string" || typeof key !== "string") return null;
  if (!Object.prototype.hasOwnProperty.call(CURRICULUM, subject)) return null;
  for (const rank of RANKS) {
    for (const c of conceptsFor(subject, rank)) {
      if (c.key === key) return { ...c, rank };
    }
  }
  return null;
}

// Map a curriculum RANK to the scoring DIFFICULTY band (the concept drill frames a
// question at its concept's rank level and rates it against that band). The five
// ranks map 1:1 onto the five bands (lib/scoring.js DIFFICULTY_ANCHORS).
export const RANK_TO_BAND = {
  elementary: "beginner",
  middle: "foundational",
  high: "intermediate",
  university: "advanced",
...
// The difficulty band a concept drill should use for a concept at `rank` (defaults
// to intermediate for an unknown rank — prototype-safe lookup).
export function bandForRank(rank) {
  return Object.prototype.hasOwnProperty.call(RANK_TO_BAND, rank) ? RANK_TO_BAND[rank] : "intermediate";
}

// The five difficulty bands in ascending order — the canonical ladder shared by the
// drill calibration below and /api/score's "≤1 band above stored level" clamp
// (mirrors lib/scoring.js DIFFICULTY_ANCHORS; RANK_TO_BAND maps onto it 1:1).
export const BAND_LADDER = ["beginner", "foundational", "intermediate", "advanced", "phd"];

// MASTERY-CALIBRATED DRILL BAND (RANKS_PLAN §6 via §12.3): the band a concept drill
// uses for a concept at `rank`, shifted by the learner's mastery STATE on that
// concept — green (mastered) stretches one band UP, red (struggling) drops one band
// DOWN (the gentler round the red warning promises), yellow/grey stay at the rank
// band. Clamped to the ladder; unknown states/ranks degrade to the plain rank band,
// so this can never widen the band range /api/score already tolerates.
export function calibratedDrillBand(rank, state) {
  const idx = BAND_LADDER.indexOf(bandForRank(rank));
  const shift = state === "green" ? 1 : state === "red" ? -1 : 0;
  return BAND_LADDER[Math.max(0, Math.min(BAND_LADDER.length - 1, idx + shift))];
}
...
// ---------------------------------------------------------------------------
// CROSS-SUBJECT PREREQUISITES — the §12.2 "future enhancement", hand-authored like
// PREREQUISITES: the OTHER-subject foundations a concept genuinely leans on (e.g.
// calculus-based physics → math derivatives, pH → math logarithms). Kept SEPARATE
// from PREREQUISITES (whose values are same-subject key strings) so existing
// consumers are untouched; values here are { subject, key } refs.
//
// ACYCLIC BY CONSTRUCTION via two hard rules (pinned by test/curriculum.test.js):
//   1. Edges point only to an EARLIER subject in SUBJECT_FOUNDATION_ORDER — math
//      underpins physics, both underpin chemistry; math never points out.
//   2. A target's rank is never HIGHER than the source concept's rank.
// Edges are deliberately sparse (1–2 per concept, only where the dependency is
// load-bearing) — this is a "brush up on the math first" nudge, not a full map.
// ---------------------------------------------------------------------------
export const SUBJECT_FOUNDATION_ORDER = ["math", "physics", "chemistry"];

export const CROSS_SUBJECT_PREREQUISITES = {
  physics: {
    // middle
    "speed_velocity_acceleration": [{ subject: "math", key: "ratios_unit_rates" }],
    // high
    "kinematics": [{ subject: "math", key: "quadratics" }, { subject: "math", key: "slope_rate_of_change" }],
    "newtons_laws": [{ subject: "math", key: "one_variable_equations" }],
    "forces_fbd": [{ subject: "math", key: "vectors_intro" }],
    "work_energy_power": [{ subject: "math", key: "algebraic_expressions" }],
    "universal_gravitation": [{ subject: "math", key: "scientific_notation" }],
    "simple_harmonic_motion": [{ subject: "math", key: "trig_graphs" }],
    "geometric_optics": [{ subject: "math", key: "angle_relationships" }, { subject: "math", key: "scale_similarity" }],
    "dc_circuits": [{ subject: "math", key: "proportional_relationships" }],
    "electric_charge_fields": [{ subject: "math", key: "scientific_notation" }],
    // university
    "vectors_kinematics_calc": [{ subject: "math", key: "derivatives" }, { subject: "math", key: "vectors_intro" }],
    "newtonian_dynamics": [{ subject: "math", key: "derivatives" }],
    "rotational_motion_torque": [{ subject: "math", key: "vectors_intro" }],
    "statics_equilibrium": [{ subject: "math", key: "systems_equations_inequalities" }],
    "oscillations_waves_adv": [{ subject: "math", key: "linear_odes" }],
    "statistical_mechanics_intro": [{ subject: "math", key: "probability_theory" }],
    "electrostatics_gauss": [{ subject: "math", key: "vector_calculus" }],
    "potential_capacitance": [{ subject: "math", key: "integrals_ftc" }],
    "induction_maxwell": [{ subject: "math", key: "vector_calculus" }],
    "ac_circuits": [{ subject: "math", key: "trig_graphs" }],
    "physical_optics": [{ subject: "math", key: "trig_identities" }],
    "special_relativity": [{ subject: "math", key: "radical_expressions" }],
    "quantum_mechanics_intro": [{ subject: "math", key: "real_complex_numbers" }],
  },
  chemistry: {
    // middle
    "properties_density": [{ subject: "math", key: "ratios_unit_rates" }],
    // high
    "mole_molar_mass": [{ subject: "math", key: "scientific_notation" }],
    "stoichiometry": [{ subject: "math", key: "proportional_relationships" }],
    "gas_laws": [{ subject: "math", key: "proportional_relationships" }],
    "solutions_concentration": [{ subject: "math", key: "ratios_unit_rates" }],
    "acids_bases_intro": [{ subject: "math", key: "logarithms" }],
    "thermochemistry_intro": [{ subject: "physics", key: "thermal_energy_transfer" }],
    "atomic_structure": [{ subject: "physics", key: "electric_charge_fields" }],
    // university
    "quantum_atomic_models": [{ subject: "physics", key: "quantum_mechanics_intro" }],
    "electron_config_quantum": [{ subject: "physics", key: "quantum_mechanics_intro" }],
    "gas_laws_kmt": [{ subject: "physics", key: "thermodynamics_kinetic" }],
    "thermodynamics_chem": [{ subject: "physics", key: "thermodynamics_kinetic" }],
    "chemical_kinetics": [{ subject: "math", key: "derivatives" }, { subject: "math", key: "first_order_odes" }],
    "chemical_equilibrium": [{ subject: "math", key: "quadratics" }],
    "acid_base_equilibria": [{ subject: "math", key: "logarithms" }],
    "electrochemistry": [{ subject: "physics", key: "electric_potential_current" }],
    "intermolecular_forces": [{ subject: "physics", key: "electric_charge_fields" }],
    "spectroscopy": [{ subject: "physics", key: "em_waves" }],
    "nuclear_chemistry_adv": [{ subject: "physics", key: "atomic_nuclear_physics" }],
  },
};

// Raw cross-subject refs ([{ subject, key }]) for a (subject, concept). [] when the
// subject has no cross map or the concept has no recorded cross roots — prototype-safe.
export function crossPrereqRefsFor(subject, conceptKey) {
  if (typeof subject !== "string" || typeof conceptKey !== "string") return [];
  if (!Object.prototype.hasOwnProperty.call(CROSS_SUBJECT_PREREQUISITES, subject)) return [];
  const m = CROSS_SUBJECT_PREREQUISITES[subject];
  return Object.prototype.hasOwnProperty.call(m, conceptKey) ? m[conceptKey] : [];
}

// Resolve a concept's cross-subject roots to full concept objects tagged with their
// OWN subject ({ subject, key, label, strand, rank }) — the concept page renders them
// as chips that navigate into the other subject's page. Unresolvable refs are dropped.
export function crossRootsFor(subject, conceptKey) {
  const out = [];
  for (const ref of crossPrereqRefsFor(subject, conceptKey)) {
    const c = ref && conceptByKey(ref.subject, ref.key);
    if (c) out.push({ subject: ref.subject, ...c });
  }
  return out;
}

// --- weak-concept resolution (the grader speaks the curriculum) ------------
// The grader now reports the concepts a learner is weak on AS CURRICULUM KEYS,
// chosen from the per-subject list we inject into its prompt (see
// subjectConceptLines + lib/groq.js). This resolver maps whatever the grader (or
// a legacy stored row) returns back onto a canonical curriculum key, so every
// "WORK ON" chip points at a real, prepared concept guide (lib/guides) instead of
// triggering an ad-hoc generation. It accepts, in priority order:
//   1. an exact curriculum key ("linear_equations") — the normal grader output;
//   2. a concept LABEL ("Linear equations") — grader/label drift;
//   3. free text ("isolating variable in linear equations") — LEGACY scores rows:
//      the concept whose every label token appears in the text, longest label wins.
// Returns the canonical key or null (caller drops nulls).

// Normalize a label/phrase for token matching: lowercase, "&" -> "and", strip
// punctuation to spaces, collapse runs. Mirrors nothing security-sensitive; purely
// for fuzzy curriculum lookup.
...
// Tokens with no discriminating power for curriculum matching — dropped from BOTH
// the concept-label index and the phrase before overlap scoring, so a shared "of"/
// "and"/"intro"/"concept" can't carry (or skew) a match.
const MATCH_STOPWORDS = new Set([
  "and", "the", "for", "with", "from", "into", "its", "their", "your", "this",
  "that", "are", "was", "how", "why", "when", "then", "than", "intro",
  "introduction", "basic", "basics", "level", "concept", "concepts", "topic",
  "topics", "problem", "problems", "learner", "student", "students", "skill",
  "skills", "understanding",
  // generic descriptive verbs that appear inside a few labels ("Representing &
  // interpreting data") but carry no topical signal — kept out so they can't
  // hijack an unrelated phrase onto that one concept.
  "interpreting", "representing",
]);

// Normalize one token for fuzzy matching: drop a single trailing plural "s" (but not
// "ss"), keeping a length-3 floor so short roots ("gas") survive. Light on purpose —
// the prefix test below absorbs the deeper inflectional/derivational variants.
function normToken(t) {
  return t.length > 3 && t.endsWith("s") && !t.endsWith("ss") ? t.slice(0, -1) : t;
}

// Content tokens of a normalized phrase: split, drop short/stopword tokens, normalize.
function contentTokens(norm) {
  const out = [];
  for (const tok of norm.split(" ")) {
    if (tok.length < 3 || MATCH_STOPWORDS.has(tok)) continue;
    out.push(normToken(tok));
  }
  return out;
}

// Two tokens "match" when equal, or when they share a >=7-char prefix — enough to
// align inflectional/derivational variants the plural-strip misses (stoichiometric ~
// stoichiometry, molecular ~ molecule, thermodynamic ~ thermodynamics) without an
// aggressive stemmer that would over-merge short roots (electric ~ electron: only a
// 6-char shared prefix, so they stay distinct).
...
// Per-subject lookup index, built once from CURRICULUM. keySet = valid keys;
// labelIndex = normalized label -> key; concepts carry their normalized label
// content tokens for the legacy free-text fallback.
const RESOLVE_INDEX = {};
for (const subject of Object.keys(CURRICULUM)) {
  const keySet = new Set();
  const labelIndex = new Map();
  const concepts = [];
  const df = new Map(); // token -> # of concepts whose label contains it (for IDF)
  for (const rank of RANKS) {
    for (const c of conceptsFor(subject, rank)) {
      keySet.add(c.key);
      const norm = normForMatch(c.label);
      labelIndex.set(norm, c.key);
      const tokens = contentTokens(norm);
      concepts.push({ key: c.key, tokens });
      for (const t of new Set(tokens)) df.set(t, (df.get(t) || 0) + 1);
    }
  }
  RESOLVE_INDEX[subject] = { keySet, labelIndex, concepts, df };
}

export function resolveConceptKey(subject, value) {
  const idx = RESOLVE_INDEX[subject];
  if (!idx || typeof value !== "string") return null;
  const raw = value.trim();
  if (!raw) return null;
  if (idx.keySet.has(raw)) return raw; // 1) already a curriculum key
  const norm = normForMatch(raw);
  if (idx.labelIndex.has(norm)) return idx.labelIndex.get(norm); // 2) exact label
  // 3) legacy free text / grader drift ("isolating variable in linear equations"):
  // pick the curriculum concept whose label content tokens OVERLAP the phrase most.
  // Overlap count dominates; ties break toward the concept the overlap covers most
  // fully (the more specific match). Zero overlap -> null, so unrelated text never
  // resolves to a bogus concept and the caller drops it (every surviving chip is a
  // real, prepared concept).
  const phraseToks = contentTokens(norm);
  if (!phraseToks.length) return null;
  let best = null;
  let bestScore = 0;
  for (const c of idx.concepts) {
    if (!c.tokens.length) continue;
    let overlap = 0;
    let weight = 0; // IDF-weighted: a distinctive label token ("work") outscores a
    for (const ct of c.tokens) {
      // common one ("force") shared across many concepts, so an ambiguous phrase
      // lands on the concept it identifies most specifically.
      if (phraseToks.some((pt) => tokensMatch(ct, pt))) {
        overlap++;
        weight += 1 / (idx.df.get(ct) || 1);
      }
    }
    if (!overlap) continue;
    // Rank by: (1) overlap COUNT — two topical tokens beat one rare token, so a
    // "frictionless" setup word can't outrank a "potential energy" match; then
    // (2) IDF weight — a distinctive token ("work") beats a common one ("force");
    // then (3) a tiny coverage term — the more specific label wins an exact tie.
    const score = overlap * 1000 + weight + (overlap / (c.tokens.length + 1)) * 1e-6;
    if (score > bestScore) {
      bestScore = score;
      best = c;
    }
...
// The label for a curriculum key (the human-readable chip text). null if unknown.
export function conceptLabel(subject, key) {
  const c = conceptByKey(subject, key);
  return c ? c.label : null;
}

// The "key — label" block injected into the grader prompt so it can pick
// weakConcepts BY KEY from the real curriculum (WIP/doctorate ranks have no
// concepts, so they're naturally excluded). One subject's concepts, all ranks.
export function subjectConceptLines(subject) {
  if (!Object.prototype.hasOwnProperty.call(CURRICULUM, subject)) return "";
  const lines = [];
  for (const rank of RANKS) {
    for (const c of conceptsFor(subject, rank)) lines.push(`${c.key} — ${c.label}`);
  }
  return lines.join("\n");
}

// The full grader-prompt block that lists the subject's curriculum concepts and
// tells the grader to pick weakConcepts BY KEY from it. "" when the subject has no
// concepts (so callers can concatenate unconditionally). Shared by /api/grade and
// /api/score so both graders see an identical instruction.
export function weakConceptPromptBlock(subject) {
  const lines = subjectConceptLines(subject);
  return lines
    ? `Curriculum concepts (choose weakConcepts BY KEY — the token before the " — "):\n${lines}\n\n`
    : "";
}
