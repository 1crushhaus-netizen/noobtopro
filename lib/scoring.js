...
export function blend(prev, suggestion, opts) {
  ...
  // A non-number OR a non-finite number (NaN/Infinity) prev means "no previous score"
  // per the contract — seed from the suggestion rather than treating it as 0 and dragging
  // the result toward zero (audit 05 P2-3).
  if (typeof prevNum !== "number" || !Number.isFinite(prevNum)) return sug;
...
// difficulty anchors (beginner 35 / intermediate 175 / advanced 245), so points are
...
// CONSERVATIVE DIAGNOSTIC PLACEMENT (FIX 7, owner decision = RETUNE). A cold 3-step
// walk is a coarse first read, so its placement must NOT pin the absolute max or over-
// promote a middling run. Instead of mapping the 0–100 path QUALITY straight onto
// 0–SCORE_MAX (×3.5 — a flawless walk landed at 350, and a ~60% walk reached ~210
// University), the quality is scaled by DIAG_PLACEMENT_CEILING instead of SCORE_MAX:
//   target = round(quality × DIAG_PLACEMENT_CEILING / 100)
// With CEILING = 290 the placement is monotonic (better reasoning → higher placement)
// but deliberately compressed:
//   quality 100 → 290  (low Doctorate / top University — clearly BELOW the 350 max)
//   quality  60 → 174  (mid HIGH, not University)
//   quality  40 → 116  (Middle)
// Practice afterward still climbs to the true top of the range; the diagnostic just
// stops handing out a max-rank or University placement from one cold sitting.
export const DIAG_PLACEMENT_CEILING = 290;

// Sum of the difficulty anchors across the diagnostic tiers (beginner 35 + intermediate
// 175 + advanced 245 = 455). diagnosticSubjectScore divides by AT LEAST this, so a MISSING
...
// rubric weighted by the question's difficulty anchor (beginner 35 /
// intermediate 175 / advanced 245 ≈ 1:5:7). Values are kept as FLOATS in [0,
...
// Anti-laundering verification gate (audit 05 P1-2): a server-graded practice attempt only
// counts toward the leaderboard "verified" badge when it is AT-OR-NEAR the learner's level
// — the question's difficulty is no more than one rank band (SCORE_MAX/5) below the
// learner's current score. This stops 5 trivial FAR-below aces from "verifying" a high
// placement; together with the below-level rating damper (updateAxisRatings) the verified
// rank reflects real at-level work, not farmed easy items. Pure + unit-testable.
//
// `calibratedDifficulty` (optional): when the route has a population-calibrated item
// difficulty for this bucket it SHOULD pass it here, so the gate uses the SAME difficulty
// the rating path (eloUpdate) uses, instead of the static band anchor. When omitted (or
// non-finite) we fall back to the band anchor EXACTLY as before — so existing callers are
// unchanged. Follow-up: app/api/score/route.js should pass the calibrated bucket difficulty
// (the same value it feeds to eloUpdate) so the verify gate and the rating agree.
export function attemptVerifies(band, score, calibratedDifficulty) {
  const d = Number.isFinite(Number(calibratedDifficulty))
    ? Number(calibratedDifficulty)
    : defaultDifficultyForBand(band);
  const s = clampSubjectScore(score) ?? 0;
  return d >= s - SCORE_MAX / 5;
}
...
export function updateAxisRatings({ prevGlicko, prevRubric, prevScore, attemptRubric, difficulty, repeatFactor = 1 } = {}) {
  ...
  // BELOW-LEVEL anti-farm damper (audit 05 P1-1). Glicko already makes a near-level ace
  // barely move you, but the tiny residual surprise (expected is clamped below 1) accumulates
  // over a long farm of FAR-below-level items — and rotating topics defeats the per-bucket
  // repeatFactor. So scale the POSITIVE step toward 0 as the question falls more than one rank
  // band below the learner's current level: full credit within 1 band, →0 by ~3 bands below.
  // At-or-above-level practice (gap ≤ 0) is unaffected, and losses are never damped (that
  // happens inside updateAxisGlicko, which only damps step > 0). This caps "ace beginner items
  // forever → University+" to roughly one band above the difficulty actually practiced.
  const lvl = clampSubjectScore(prevScore) ?? subjectScoreFromGlicko(base);
  const d = Number.isFinite(Number(difficulty)) ? Number(difficulty) : DIFFICULTY_ANCHORS.intermediate;
  const gap = (Number.isFinite(lvl) ? lvl : 0) - d;
  const ONE_BAND = SCORE_MAX / 5; // 70
  const belowLevelFactor = gap <= ONE_BAND ? 1 : clamp(1 - (gap - ONE_BAND) / (2 * ONE_BAND), 0, 1);
  const rf = clamp(Number.isFinite(Number(repeatFactor)) ? Number(repeatFactor) : 1, 0, 1) * belowLevelFactor;
  const next = {};
  for (const k of RUBRIC_KEYS) next[k] = updateAxisGlicko(base[k], rub[k] / RUBRIC_MAX, difficulty, rf);
  return {
    glicko: next,
...
export function diagnosticSeedFromReasoning(perQuestion, { pathWeighted = false } = {}) {
  ...
  // QUALITY (0–100). FIX 7 (RETUNE): map it onto the subject scale CONSERVATIVELY —
  // ×(DIAG_PLACEMENT_CEILING/100) rather than ×(SCORE_MAX/100), so a flawless cold
  // walk lands at the ceiling (low Doctorate, below max) and a ~60% walk at mid-High,
  // not the top of the range. Monotonic in quality.
  const quality = pathWeighted ? diagnosticPathScore(qs) : diagnosticSubjectScore(qs);
  const target = Math.round((quality * DIAG_PLACEMENT_CEILING) / 100);
