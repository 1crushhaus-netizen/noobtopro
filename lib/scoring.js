...
// the rated opponent (its difficulty is its rating, on the same 0–100 scale → Glicko
...
// RUBRIC_WEIGHTS-weighted mean of the 9 axis ratings, squashed to 0–100 — so acing an
...
// τ); we convert only at the boundaries: a question difficulty (0–100) → opponent
// rating via toGlickoRating, and a rating → 0–100 display via a LOGISTIC squash (a
...
const DISPLAY_D = 275;         // logistic display spread (rating → 0–100)
...
// Glicko rating → integer 0–100 display score (the logistic squash). Bounded by
// construction; r=1500 → 50.
export function scoreFromRating(rating) {
  const r = Number(rating);
  if (!Number.isFinite(r)) return 50;
  return clamp(Math.round(100 * sigmoid((clamp(r, RATING_LO, RATING_HI) - GLICKO_R0) / DISPLAY_D)), 0, 100);
}

// 0–100 score → Glicko rating: the EXACT (unrounded) inverse of scoreFromRating. Used
// to seed legacy users (continuity) and to rate the question as an opponent.
export function ratingFromScore(score) {
  const s = Number(score);
  const p = clamp(Number.isFinite(s) ? s / 100 : 0.5, E_LO, E_HI);
  return GLICKO_R0 + DISPLAY_D * Math.log(p / (1 - p));
}

// Question difficulty (0–100) → opponent Glicko rating (rated by the same inverse, so
// an at-level item d=50 vs a score-50 learner yields expected 0.5).
export function toGlickoRating(difficulty) {
  return ratingFromScore(Number.isFinite(Number(difficulty)) ? Number(difficulty) : 50);
}
...
// One attempt's update for a single axis: opponent = the question difficulty (0–100),
...
export function seedGlickoFromRubric(rubric, fallbackScore = null) {
  ...
  const fs = clampScore(fallbackScore);
  if (rubric && typeof rubric === "object") {
    const r = normalizeRubricFloat(rubric);
    const raw = {};
    for (const k of RUBRIC_KEYS) raw[k] = ratingFromScore((r[k] / RUBRIC_MAX) * 100);
    const out = {};
    if (fs == null) {
...
// The DERIVED subject score (int 0–100) → scores.score / totalPoints / phdIndex / leaderboard.
...
export function radarFromGlicko(glicko) {
  const st = normalizeGlickoState(glicko);
  const out = {};
  for (const k of RUBRIC_KEYS) out[k] = clamp((scoreFromRating(st[k].rating) / 100) * RUBRIC_MAX, 0, RUBRIC_MAX);
  return out;
}
 ...
 *   difficulty: the question difficulty (0–100; calibrated bucket server-side, band anchor for guests).
 *   repeatFactor: anti-farm gain damper (∈ [0,1]).
 * @returns {{glicko, rubric, score, expected}}  new state + derived radar (0–4) + subject score (0–100)
...
export function diagnosticSeedFromReasoning(perQuestion) {
  const qs = (Array.isArray(perQuestion) ? perQuestion : []).filter((q) => q && q.rubric != null);
  if (qs.length === 0) {
    const empty = emptyGlickoState();
    return { glicko: empty, rubric: radarFromGlicko(empty), score: 0 };
  }
  // Placement target = the difficulty-weighted mean of the per-tier reasoning scores.
  const target = diagnosticSubjectScore(qs);
