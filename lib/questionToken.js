...
// `uid` BINDS the token to the account it was issued to (audit F2): /api/score rejects a
// token whose signed uid ≠ the scoring account, so a token can't be pooled/shared across
// accounts. Signed only for authenticated generation (guests have no uid); absent → the
// bind check is skipped (backward-compatible with tokens issued before this field existed).
const PAYLOAD_FIELDS = ["subject", "question", "targetConcept", "difficulty", "topicSlug", "reasoningSurface", "trap", "conceptKey", "uid", "jti", "iat", "exp"];
