...
// Read a request body with a HARD byte ceiling, streaming the body and aborting as
// soon as the cap is crossed so at most `maxBytes` is ever buffered (Content-Length can
// be absent or spoofed, so we can't trust it alone). Returns the raw bytes (Uint8Array).
//   - throws an error with .code === "BODY_TOO_LARGE" when the body exceeds maxBytes.
async function readBytesLimited(req, maxBytes) {
...
// Read and JSON-parse a request body with a HARD byte ceiling (see readBytesLimited).
//   - throws .code === "BODY_TOO_LARGE" when the body exceeds maxBytes (caller → 413),
...
// Read a request body as RAW TEXT with a HARD byte ceiling (see readBytesLimited).
// For paths that need the exact raw string rather than parsed JSON — e.g. the Polar
// webhook, whose Standard-Webhooks signature is computed over the raw body (audit P1-4:
// previously a bare `req.text()` with no cap, the one public endpoint missing the ceiling).
//   - throws .code === "BODY_TOO_LARGE" when the body exceeds maxBytes (caller → 413).
export async function readTextLimited(req, maxBytes = MAX_BODY_BYTES_TEXT) {
  const bytes = await readBytesLimited(req, maxBytes);
