...
// string. A legacy bare "granted"/"denied" string is still read for back-compat.
export const CONSENT_STORAGE_KEY = "noobtopro:consent";

// Normalise a human date like "19 June 2026" (the lib/legal.js `lastUpdated` format) to an ISO
// "YYYY-MM-DD" string for lexicographic version comparison. Returns null if it can't be parsed.
...
// CONSENT_VERSION is TIED to the policy revision date (lib/legal.js `lastUpdated`) so the two can
// never silently drift apart. A stored consent older than this version is treated as STALE and the
// banner re-asks (see readConsentChoice below). ePrivacy Art. 5(3) / EDPB: a MATERIAL change to the
// cookie/analytics disclosure REQUIRES fresh consent — so whenever such a change ships you MUST bump
// `lastUpdated` (which flows into this constant) to force every returning visitor to re-consent.
// Conversely, do NOT set this AHEAD of the current `lastUpdated`: that would gratuitously invalidate
// consents that are still valid. The value is an ISO date, which sorts lexicographically =
// chronologically, so a plain string compare is a correct "same-or-newer" test.
export const CONSENT_VERSION = toIsoDate(LEGAL.lastUpdated) || "2026-06-19";
