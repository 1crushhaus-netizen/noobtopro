...
// The score floor of the rank band a depth score falls into (bands are 70 wide,
// mirroring band(): Elementary 0–70 … Doctorate 280–350). Used to grandfather the
// mastery blend so it can refine the number WITHIN a learner's earned band but never
// demote below it.
function depthBandFloor(depth) {
  const n = clampSubjectScore(depth) ?? 0;
  if (n < 70) return 0;
  if (n < 140) return 70;
  if (n < 210) return 140;
  if (n < 280) return 210;
...
// The displayed subject score: the depth score scaled by mastery coverage, but FLOORED
// at the band the depth earned, so demonstrated reasoning is never erased. A real
// University reasoner with no mastered concepts — e.g. any account before concept-tagged
// practice has run, or a failed/empty mastery load — keeps their rank instead of cratering
// to Elementary; coverage only raises the number from the band floor toward the full depth
// as concepts go green. Integer in [0, SCORE_MAX]; clampSubjectScore coerces garbage safely.
export function effectiveSubjectScore(depthScore, mastery, subject) {
  const depth = clampSubjectScore(depthScore) ?? 0;
  const blended = Math.round(depth * coverageFraction(mastery, subject));
  return Math.max(depthBandFloor(depth), blended);
}
...
export function subjectRankView(depthScore, mastery, subject) {
  const depth = clampSubjectScore(depthScore) ?? 0;
  const { green, total } = masteryCounts(mastery, subject);
  // Reuse the floored blend so the chip's rank can never fall below the depth-earned band.
  const effective = effectiveSubjectScore(depth, mastery, subject);
  return {
    depth,
    effective,
    green,
