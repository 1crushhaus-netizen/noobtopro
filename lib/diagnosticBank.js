...
// intermediate, advanced. `topicSlug` is a valid lib/taxonomy.js slug. `conceptKey`
// is the lib/curriculum.js concept this question evidences — it feeds the per-concept
// MASTERY coloring (RANKS_PLAN §12.1): the diagnostic colors exactly the concepts it
// tests (direct-only, no propagation). To edit the placement, edit this list (and
// keep one question per subject × level; test/diagnostic-bank.test.js pins the shape).
// ---------------------------------------------------------------------------

export const DIAGNOSTIC_BANK = [
  // ----- MATHEMATICS -----
  {
    subject: "math", difficulty: "beginner", conceptKey: "ratios_unit_rates", topic: "Ratios & proportions", topicSlug: "arithmetic_number",
  ...
  {
    subject: "math", difficulty: "intermediate", conceptKey: "rational_expressions", topic: "Rational equations", topicSlug: "algebra",
  ...
  {
    subject: "math", difficulty: "advanced", conceptKey: "sequences_series_hs", topic: "Proof & reasoning", topicSlug: "proof_reasoning",
  ...
  // ----- PHYSICS -----
  {
    subject: "physics", difficulty: "beginner", conceptKey: "balanced_unbalanced_forces", topic: "Forces", topicSlug: "kinematics_dynamics",
  ...
  {
    subject: "physics", difficulty: "intermediate", conceptKey: "conservation_energy", topic: "Energy conservation", topicSlug: "energy_momentum",
  ...
  {
    subject: "physics", difficulty: "advanced", conceptKey: "kinematics", topic: "Motion under gravity", topicSlug: "kinematics_dynamics",
  ...
  // ----- CHEMISTRY -----
  {
    subject: "chemistry", difficulty: "beginner", conceptKey: "mole_molar_mass", topic: "Composition by mass", topicSlug: "stoichiometry",
  ...
  {
    subject: "chemistry", difficulty: "intermediate", conceptKey: "stoichiometry", topic: "Limiting reagents", topicSlug: "stoichiometry",
  ...
  {
    subject: "chemistry", difficulty: "advanced", conceptKey: "stoichiometry_adv", topic: "Limiting reagents from mass", topicSlug: "stoichiometry",
...
// SERVER-SIDE source of truth for which curriculum concept a diagnostic slot
// evidences (mastery coloring, RANKS_PLAN §12.1) — derived from the bank by
// (subject, difficulty) exactly like the reasoning surface above, never trusted
// from the request body, so a crafted payload can't color (or farm) arbitrary
// concepts. Unknown slot → null (no mastery update).
export function diagnosticConceptFor(subject, difficulty) {
  const q = BANK_BY_SLOT.get(`${subject}:${difficulty}`);
  return (q && q.conceptKey) || null;
}

// SERVER-SIDE source of truth for the QUESTION TEXT of a diagnostic slot. The
// grader uses THIS, never the request body's question — the diagnostic is the
// standardized placement, so a client substituting its own (easier) question
// for a slot must not be able to earn that slot's baseline credit. Unknown
// slot → null (the answer is skipped).
export function diagnosticQuestionFor(subject, difficulty) {
  const q = BANK_BY_SLOT.get(`${subject}:${difficulty}`);
