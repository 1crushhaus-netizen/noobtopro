...
export async function deleteAllUserData() {
  ...
  const sb = getSupabase();
  const { error: aErr } = await sb.from("attempts").delete().eq("user_id", uid);
  const { error: sErr } = await sb.from("scores").delete().eq("user_id", uid);
  if (aErr || sErr) throw new Error((aErr || sErr).message || "Could not delete your data.");
  writeLocal({ scores: null, history: [] });
  return { ok: true };
}
