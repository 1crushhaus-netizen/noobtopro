...
import { getSupabase, ensureSupabase } from "@/lib/supabase";
import { clampScore, clampSubjectScore } from "@/lib/scoring";
import { normalizeMasteryMap, applyMasteryUpdates, masteryMapFromRows } from "@/lib/mastery";
import { withdrawalWindowEnd } from "@/lib/consent";
...
// getSupabase), so the data layer works in both the app and tests unchanged.
...
// thin row read.
//
// Also returns `withdrawalUntil` (ISO string | null): the end of the EU 14-day withdrawal
// window, anchored on the latest recorded Art. 16(a) consent (billing_audit, SELECT-own).
// The dashboard shows the "Withdraw from contract here" control only while a Pro user is
// still inside this window; the /api/account/withdraw route re-checks it authoritatively.
export async function loadSubscription() {
  const uid = await activeUserId();
  if (!uid) return { subscription: null, withdrawalUntil: null };
  const sb = await sbClient();
  if (!sb) return { subscription: null, withdrawalUntil: null };
  ...
  try {
    ...
    if (!error && data && data.created_at) {
      const end = withdrawalWindowEnd(data.created_at);
      withdrawalUntil = end ? end.toISOString() : null;
    }
  } catch {
    /* no audit table / no consent yet → no window */
  }
  return { subscription, withdrawalUntil };
}

// Exercise the EU "Withdraw from contract here" right (CRD Art. 11a) for a signed-in Pro
// user: POST /api/account/withdraw with the session bearer token. The server re-verifies the
// 14-day window, terminates the subscription immediately, and issues the pro-rata refund.
// Returns the on-screen confirmation { ok, withdrawnAt, refund } on success; throws (with the
// server's message) so the caller can surface why (e.g. the window has closed).
export async function withdrawFromContract() {
  const sb = await sbClient();
  if (!sb) throw new Error("Withdrawal is not available right now.");
  let token = null;
  try {
    const { data } = await sb.auth.getSession();
    token = (data && data.session && data.session.access_token) || null;
  ...
  if (!token) throw new Error("Please sign in again to withdraw.");
  ...
  if (!res.ok) {
    let msg = "Could not complete your withdrawal. Please try again.";
    try {
...
export async function loadState() {
  ...
  const sb = await sbClient();
  // `scores` stays client-readable (SELECT-own RLS). The FREE attempt history (the
  // "problems graded" count + the "why your reasoning moved" rationale) comes from
  // /api/history — direct client SELECT on `attempts` is revoked because "Progress trends"
  // is a paid Pro feature (db/migrations/0024). The history here carries NO cumulative
  // total; the Pro trend chart series is loaded on demand via loadTrends() -> /api/trends.
  let token = null;
  try {
    const { data } = await sb.auth.getSession();
    token = (data && data.session && data.session.access_token) || null;
  } catch {
    /* fall through; the history fetch fails and is surfaced below */
  }
  ...
  // indistinguishable from a brand-new user, and the caller would otherwise wipe a
  // signed-in user back to the intro screen (and risk overwriting real scores).
  if (scoreRes.error) return { error: scoreRes.error };
  if (!historyRes || !historyRes.ok) return { error: new Error("Could not load your history.") };
  let history;
  try {
    const j = await historyRes.json();
    history = Array.isArray(j && j.history) ? j.history : [];
  } catch {
    return { error: new Error("Could not load your history.") };
  }
  ...
  // /api/history already returns chronological (oldest→newest) order.
  return { scores, history };
}

// Load the learner's trend chart series (cumulative points + per-attempt deltas over time).
// Signed-in: "Progress trends" is a PRO feature, so the series comes from /api/trends, which
// verifies the JWT AND the Pro entitlement before returning rows via the service role (direct
// client SELECT on `attempts` is revoked — db/migrations/0024). A non-Pro caller gets 402 (the
// UI already gates the "See trends" button, so this is the server backstop). Guest: built from
// the local history (the guest dashboard is sign-in-gated). Returns { trends } or { error }.
export async function loadTrends() {
  const uid = await activeUserId();
  if (!uid) {
    const local = sanitizeState(readLocal());
    return { trends: local.history || [] };
  }
  const sb = await sbClient();
  let token = null;
  try {
  ...
  if (!token) return { error: "Please sign in again to see your trends." };
  let res;
  try {
    res = await fetch("/api/trends", {
      method: "POST",
      headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
      body: "{}",
    });
  } catch {
    return { error: "Could not load your trends." };
  }
  if (!res.ok) {
    let msg = "Could not load your trends.";
    try {
      const j = await res.json();
      if (j && j.error) msg = j.error;
    } catch {
  ...
  try {
    const j = await res.json();
    return { trends: Array.isArray(j && j.trends) ? j.trends : [] };
  } catch {
    return { error: "Could not load your trends." };
  }
}
...
// Download a copy of everything the account holds (GDPR access/portability). POSTs to
// /api/account/export with the session bearer token and returns the JSON bundle; the caller
// turns it into a file download. Throws (with the server's message) on failure. Guests have no
// server-side data — their progress already lives only in this browser's localStorage.
export async function exportMyData() {
  const sb = await sbClient();
  if (!sb) throw new Error("Sign in to export your account data.");
  let token = null;
  try {
    const { data } = await sb.auth.getSession();
    token = (data && data.session && data.session.access_token) || null;
  ...
  if (!res.ok) {
    let msg = "Could not export your data. Please try again.";
    try {
      const j = await res.json();
      if (j && j.error) msg = j.error;
    } catch {
