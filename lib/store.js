...
export async function saveProgress(scores, evt) {
  ...
  if (!uid) {
    ...
    // reporting success — mirrors the signed-in path, so the caller shows a banner
    // rather than silently losing the attempt. Throwing keeps in-memory React state
    // as the source of truth until the next successful save.
  ...
  const p_scores = Object.keys(scores).map((subject) => ({
    ...
    // Client-side defense: cap the element count so a corrupt/oversized in-memory
    // copy can't push an unbounded array at the DB (the RPC caps it separately).
    weak_concepts: (Array.isArray(scores[subject].weakConcepts) ? scores[subject].weakConcepts : []).slice(0, 64),
