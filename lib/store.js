...
// attempt_reviews row (snake_case) -> a unified review item the Review UI renders.
...
// Load the learner's reviewable past answers (newest first). Signed-in: read their OWN
// attempt_reviews (RLS-scoped) directly. Guest: from the review detail embedded in the
// local history. Returns { reviews } or { error }.
export async function loadReviews() {
  ...
  if (uid) {
    ...
    return { reviews: (data || []).map(reviewRowToItem) };
  }
...
async function _migrateGuestToAccount() {
  ...
  // and if the DB predates migration 0010 (PGRST202: no 3-arg signature), retry
  // without it so the scores/attempts migration still succeeds.
  ...
  let { error } = await sb.rpc("migrate_guest_data", hasMastery ? { p_scores, p_attempts, p_mastery } : { p_scores, p_attempts });
  if (error && hasMastery && error.code === "PGRST202") {
    masteryMigrated = false;
    ({ error } = await sb.rpc("migrate_guest_data", { p_scores, p_attempts }));
  }
  if (error) return { migrated: false, error };
  // The RPC is atomic + idempotent, so the guest copy is safe to clear now. If the
  // mastery map could NOT ride along (pre-0010 DB → 2-arg fallback), keep it in the
  // local blob instead of destroying it — it costs a few KB and stays recoverable.
