...
// The WRITE path is the Polar webhook (follow-up change) which records the latest
// subscription state via the service-role upsert_subscription RPC (see db/schema.sql).
//
// "Is Pro" is decided HERE (one place), not as a SQL CHECK enum, so an unrecognized
// future Polar status simply reads as non-Pro instead of breaking the webhook write.
//
 ...
 * Pro = an allow-listed active status AND not past the paid period (a missed/late revoke
 * webhook still can't extend access past current_period_end). `now` is injectable for tests.
 */
