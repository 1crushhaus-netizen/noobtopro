# Terms of Service — noobtopro

> **DRAFT for counsel review — not legal advice.** Fill every `[PLACEHOLDER]`. The EU/UK
> consumer carve-outs in §§11, 12, 15 are **mandatory** (Directive 93/13/EEC; Rome I Art. 6;
> Brussels Ibis Arts. 17–19; UK CRA 2015 Sch. 2 + s. 74) — do not delete them. Cross-refs:
> `refund-and-cancellation-policy.md`, `subscription-and-billing-terms.md`, `privacy-policy.md`.

**Last updated:** [EFFECTIVE DATE] · **Version:** [e.g. 2026-1]

These Terms of Service ("Terms") are a binding agreement between you and **[Company Legal
Name]**, established in **[EU/EEA Member State]**, registered office **[Registered Address]**,
company number **[Reg. No.]** ("noobtopro", "we", "us"). They govern your access to and use of
the noobtopro website, application, and related services (the "Service").

**If you are a consumer in the EU/EEA or the UK, nothing in these Terms removes the protections
of your local mandatory consumer law or your right to bring proceedings in the courts of the
place where you live** — see §15.

## 1. Acceptance (how the contract is formed)
You accept these Terms when you click "I agree" (or check the acceptance box, or select "Sign
in", "Create account", or "Subscribe" presented next to a clear statement that doing so means
you accept). We log your acceptance (version, date, time). These Terms incorporate our
**[Acceptable Use Policy](/aup)**, **[Privacy Policy](/privacy)**, and
**[Refund & Cancellation Policy](/refunds)**.

> *[COUNSEL/eng — clickwrap: US enforceability and good practice everywhere require conspicuous
> notice + an affirmative, logged act of assent. Avoid browsewrap.]*

## 2. Eligibility (adults only, 18+)
The Service is an **adults-only service**. You may only create an account or use the Service if
you are **at least 18 years old** (or older, if the age of majority where you live is higher).
You represent and warrant that you meet this requirement. We ask you to confirm your **date of
birth** to verify it, we do not knowingly permit anyone under 18 to use the Service, and we may
suspend or terminate any account we reasonably believe belongs to a person under 18.

## 3. Your account and security
Sign in with **Google** (via our authentication provider, **Supabase**). You are responsible
for activity under your account and for keeping your details accurate. Notify us at
**[security@DOMAIN]** of any unauthorised use. You may delete your account at any time (§13;
Privacy Policy), which permanently removes your account and associated personal data, subject
to records we must retain by law.

## 4. The Service — what it is, and the "rank" disclaimer
**4.1** You submit your reasoning for **maths, physics, and chemistry** problems (typed answers
and/or **photos of handwritten work**), and the Service uses a third-party large-language-model
provider (**Groq**) to grade the reasoning and estimate a relative skill score on a **0–350
scale** (your "rank").

**4.2 The rank is NOT an assessment, qualification, or certification.** It is a self-calibrating
learning signal only — not an accredited exam score, grade, diploma, certificate, qualification,
or any professional/academic/psychometric assessment, and not a statement of your standing or
ability. **You must not rely on the rank, or on any grade, feedback, or generated content, for
any academic, admissions, employment, examination, credentialing, financial, or other
consequential decision.** AI-assisted grading is probabilistic and can be wrong, incomplete, or
out of date. See `ai-transparency-notice.md`.

**4.3 Free and Pro tiers.** A free tier (with usage limits, e.g. a daily practice cap) and a
paid "Pro" tier, as described in the Service.

## 5. Your content and the licence you grant us
**5.1** You retain ownership of the answers, working, photographs, and other material you submit
("User Content"). **5.2** You grant us a **worldwide, non-exclusive, royalty-free licence to
host, store, reproduce, transmit, process, and display your User Content solely to the extent
necessary to operate, provide, secure, and improve the Service** — including transmitting it to
our AI grading provider (Groq) to produce a grade. The licence ends when your User Content is
deleted from our active systems (routine backups may persist briefly; we may retain content
where required by law or to resolve disputes). **5.3** You represent that you own or have rights
to your User Content, that it infringes no third-party rights, and that **it contains no personal
or sensitive information about any other person** (do not photograph other people or their
identifying details). **5.4** Feedback you send us may be used without obligation.

> *[COUNSEL: keep §5.2 minimal/operational. If User Content is ever used to train models
> (yours or Groq's), that needs separate disclosure and likely an opt-in basis — confirm
> Groq's data-use terms.]*

## 6. Acceptable use
Governed by our **[Acceptable Use Policy](/aup)**. Without limiting it, you will not: game or
reverse-engineer the scoring/ranking/rate-limiting/anti-abuse systems; probe, scrape, overload,
or disrupt the Service or circumvent access controls or another user's data; use the Service
unlawfully or submit unlawful/infringing/harmful content; or use automated access except as we
permit.

## 7. Pro subscription, billing, and auto-renewal
The "Pro" tier is a recurring subscription (currently **€9.99/month**, or the price shown at
checkout). **Payments are processed by [Polar](https://polar.sh) as our Merchant of Record (the
seller of record for your transaction)** — see `subscription-and-billing-terms.md`. Subscriptions
**renew automatically** until you cancel. Full pricing, billing, auto-renewal, withdrawal, and
refund terms are in the **[Refund & Cancellation Policy](/refunds)**, incorporated here. Manage
or cancel any time from **Dashboard → Manage subscription**.

## 8. Intellectual property
The Service (software, curriculum, content, the "noobtopro" name and logos, and the rank
methodology) is owned by **[Company Legal Name]** or its licensors. We grant you a personal,
limited, non-exclusive, non-transferable, revocable licence for your own non-commercial
educational use. All rights not expressly granted are reserved.

## 9. Third-party services
The Service relies on **Polar** (payments/MoR), **Groq** (AI grading), **Supabase** (auth/db),
**Google** (sign-in), and **Vercel** (hosting/analytics). Their processing of personal data is
described in our [Privacy Policy](/privacy). This does not affect any liability we have to you
as a consumer under mandatory law.

## 10. Disclaimers ("as is")
To the **maximum extent permitted by applicable law**, the Service is provided "as is" and "as
available" and we disclaim all warranties (merchantability, fitness, accuracy, non-infringement),
and do not warrant that grades, ranks, or feedback are accurate or that the Service is
uninterrupted or error-free. **EU/EEA & UK consumers — this does not exclude rights or
guarantees you have under mandatory consumer law** (see §11).

## 11. Limitation of liability
**11.1 Never excluded.** Nothing excludes or limits our liability for (a) death or personal
injury from our negligence; (b) fraud or fraudulent misrepresentation; (c) gross negligence or
wilful misconduct; (d) anything that cannot be excluded under the law of your country of
residence; and (e) for consumers, your mandatory legal rights and remedies.
**11.2 Cap (where permitted).** Subject to 11.1 and to the maximum extent permitted, our total
aggregate liability will not exceed the greater of (i) the amounts you paid us in the 12 months
before the event, or (ii) **[EUR 50]**.
**11.3 Excluded losses (where permitted).** Subject to 11.1, we are not liable for indirect,
incidental, special, consequential, or punitive damages, or loss of profits/revenue/data/
goodwill — including any decision made in reliance on a rank, grade, or generated content.
**11.4 Consumer carve-out (overrides).** If you are a consumer, 11.2–11.3 apply only so far as
permitted by your local mandatory consumer law; we are responsible for foreseeable loss we
cause by breaking this contract or failing to use reasonable care and skill, and do not exclude
liability we may not exclude.

## 12. Indemnity
**Business users** indemnify us against third-party claims arising from their breach, User
Content, or misuse. **Consumers** are responsible only for foreseeable loss caused by their
breaking these Terms or misusing the Service, to the extent permitted by local mandatory law —
no broader indemnity is asked.

## 13. Suspension and termination
You may stop using the Service and delete your account at any time. We may suspend or terminate
access for material/repeated breach, legal requirement, or to protect the Service/users —
proportionately and, where practicable, with notice and a chance to cure. Sections that should
survive (e.g. 5.3, 8, 10–12, 14–16) survive. Termination does not affect a consumer's accrued
statutory rights, including any refund due.

## 14. Changes to these Terms
For **material changes** we give **reasonable advance notice** (e.g. email or in-Service) before
they take effect, update the version/date, and — where required — seek consent; otherwise
continued use after the effective date is acceptance. You may stop using the Service and cancel
before a material change takes effect.

## 15. Governing law and dispute resolution
**15.1** These Terms are governed by the laws of **[Home Jurisdiction — EU/EEA Member State of
establishment]**, without regard to conflict-of-laws rules.
**15.2 EU/EEA consumers.** The choice of law does **not** deprive you of the mandatory consumer
protections of your country of residence (Rome I Art. 6). You may sue us **in [Home
Jurisdiction] or in the courts of the EU/EEA Member State where you live, and we will sue you
only in the courts where you live** (Brussels Ibis Arts. 17–19).
**15.3 UK consumers.** Nothing removes mandatory UK consumer law (CRA 2015, incl. s. 74). You
may bring proceedings where you live.
**15.4 US users — arbitration & class-action waiver (US-only).** *[COUNSEL — optional; decide
given mass-arbitration cost. If included: binding individual arbitration by [AAA/JAMS], jury/
class waiver, a 30-day opt-out via [legal@DOMAIN], small-claims carve-out. **Does NOT apply to
EU/EEA or UK consumers.**]*
**15.5 Everyone else.** Courts of **[Home Jurisdiction]**, without prejudice to your local
mandatory protections.

## 16. Complaints and dispute resolution (ADR)
Contact us first at **[support@DOMAIN]**. **EU/EEA consumers** may use an alternative dispute
resolution body via your national consumer authority or the European Consumer Centre (ECC)
network (**eccnet.eu**). *[COUNSEL: do NOT include the old EU ODR-platform link — it was
discontinued 20 July 2025 and Reg. 524/2013 repealed; displaying it can be a misleading
practice. State whether you commit to a named ADR entity per Art. 13 Dir. 2013/11/EU.]*
**UK consumers** may use a certified ADR provider (gov.uk/consumer-protection-rights).

## 17. Miscellaneous
Severability; no waiver; assignment (you may not assign without consent; we may assign without
reducing your consumer rights, with notice); entire agreement (with the AUP, Privacy, and
Refund policies) — but this excludes neither liability for fraudulent misrepresentation nor your
mandatory consumer rights; force majeure (without prejudice to consumer rights); language
(**[English]** prevails except where local law requires the local-language version for consumers).

## 18. Contact
**[Company Legal Name]**, **[Registered Address]** · support: **[support@DOMAIN]** · legal:
**[legal@DOMAIN]** · security: **[security@DOMAIN]** · **[VAT/Company number]**.
