...
>
> New here? Jump to [**Where to start (next task: Elo ranking + consistent grading + leaderboard)**](#where-to-start-next-task-elo-ranking--consistent-grading--leaderboard).
...
- ⭐ [**Where to start (next task: Elo ranking + consistent grading + leaderboard)**](#where-to-start-next-task-elo-ranking--consistent-grading--leaderboard)
...
## Where to start (next task: Elo ranking + consistent grading + leaderboard)

> **Shipped & live (all merged):** Concept Hub (PR #29–#31) · "Prove it" diagnostic (PR #32) · **server-authoritative scoring + reasoning radar** (PR #34/#35) · **fleet-audit fixes** (PR #36) · **diagnostic → 6 questions** (PR #37) · **durable per-account rate limiter** (PR #38). **Live DB migrations applied:** `0001a`, `0001b`, `0002`, `0003` (so the live DB == `db/schema.sql`). Each round was gated by an adversarial pre-merge review.

**Two tasks remain — build them in order as PR 3 then PR 4.** The owner has **already chosen the design** (also captured in the `ranking-grading-direction` auto-memory). **Build to it; don't re-litigate.** Both touch the scoring trust boundary, so follow the security dev-loop at the end of this section.

### PR 3 — Elo ranking + consistent/explainable grading + 5 ranks + Profile leaderboard
This replaces the demonstrative score with a real, gaming-resistant rating. Four parts:

1. **Rating engine — item-as-opponent Elo with self-calibrating difficulty.** The *question* is the rated opponent. Keep the per-subject rating on the existing **0–100** scale (reuses the bands + all UI). Give each question a difficulty on the same scale and persist it so it **auto-calibrates from the population** ("adjusts based on the users"): `expected = 1/(1+10^((difficulty−rating)/SCALE))`; `outcome = gradedReasoning/100 ∈ [0,1]`; `rating += K·(outcome−expected)` and `difficulty −= k·(outcome−expected)` (smaller `k`). `K` shrinks with the learner's attempt count (high/provisional → stable). A low outcome on an at-level item therefore **loses** rating — non-additive by construction. Evolve `lib/scoring.js`'s existing `blend()` (already has a `SCALE`, anchors, and a surprise term) into this; persist difficulty per **(subject, topic, band)** bucket (questions are generated fresh, so there's no stable per-question id — calibrate the bucket) in a new table.
2. **5 ranks via FIXED score bands** at launch — reuse the existing `band()` names (Absolute beginner → Foundational → Intermediate → Advanced → PhD-level) as the 5 ranks. **Architect for percentile-recut tiers** (the full "Hybrid" option) as a later flip once there's a user base, but **ship fixed bands now** (owner's explicit choice).
3. **Grading: extremely consistent + explainable + anti-gaming.**
   - **Pre-grader docking gate** (deterministic, NO LLM call): empty / "idk"/"dunno"/"pass" / too-short-with-off-topic / gibberish → force a low outcome + all-zero rubric. This *docks* (doesn't just withhold) and is the anti-"idk" lever.
   - **Prompt hardening** in `PRACTICE_GRADE_SYS`/`DIAG_GRADE_SYS`: 2–3 **anchored exemplars per rubric dimension** (concrete 0/2/4 answers), **rationale BEFORE the number**, explicit penalty clauses for **buzzword-stuffing / unsupported jargon / padding** with length-normalization.
   - **Determinism + reconciliation:** grade at **temperature 0** (or sample 3× and take the median for the diagnostic); add a server-side check so a `reasoningScore` that contradicts its rubric mean is reconciled toward it (with a unit test).
   - **Explainability:** persist a one-line **"why your rank moved"** rationale per attempt and show it (the Profile/feedback must say *why* points were gained or docked).
   - **Gold set:** a tiny hand-graded fixture run on every prompt/model change with a CI tolerance test.
4. **Leaderboard on Profile** (per-subject + overall). ⚠️ **New, security-sensitive surface** — showing other users' ranks means exposing data beyond the current own-rows-only RLS. Owner default: **opt-out, display-name + rank only** (NO email, NO per-attempt data), via a `SECURITY DEFINER` read that returns top-N + the caller's own position; add a min-attempts gate + ties by earliest. *(Owner may instead pick friends-only or anonymous tier-only — confirm before building.)* Review it exactly like `/api/score`/`/api/admin`.

**Key files for PR 3:** `lib/scoring.js` (Elo engine + rank bands + reconciliation, pure) · `lib/groq.js` (anchored-exemplar prompts) · a new pre-grader docking gate (`lib/gradeInput.js` or a new module) · `app/api/score/route.js` (apply engine + gate + reconciliation; persist the rationale + item difficulty) · `db/schema.sql` + a new migration (per-(subject,topic,band) difficulty table; a leaderboard read RPC; a `rank`/rating + leaderboard-opt-out column or a `profiles` row) · `components/ProfileTab.jsx` (rank badge + leaderboard) · `components/ProgressDashboard.jsx`.
...
### Backlog (lower priority — see [§17](#17-roadmap--known-limitations))
Percentile-recut rank tiers (the PR 3 follow-on); per-item IRT/Elo (vs band-bucket difficulty); Concept Hub v1.1 (`pg_trgm` fuzzy search, `tags[]`, open the hub to undiagnosed guests, `times_opened` popularity); nonce-based strict CSP; GitHub/Discord sign-in activation; ESLint in CI; `delete_user_data` doesn't delete the `auth.users` account.
