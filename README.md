...
> - ⚠️ **The DB migration ([`db/migrations/0001_server_authoritative_scoring.sql`](db/migrations/0001_server_authoritative_scoring.sql)) is NOT yet applied to the live project.** It is BREAKING for the old deployed client (removes the client write path), so apply it **together with / right after** deploying this branch, then re-check Supabase advisors. `db/schema.sql` is updated to the final state.
...
db/
  ...
  migrations/          Ordered delta migrations against a live DB (0001 = server-authoritative scoring)
