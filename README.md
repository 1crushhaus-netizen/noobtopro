...
>
> **Live DB migrations `0001a`–`0008` are ALL applied** to the Supabase project (`vwvhgnlgubctrgksyohr`), so the live DB == `db/schema.sql`. Advisor baseline is the documented/accepted set (§17). **454 tests across 33 files + `npm run build` are green.** The Concept Hub has **9 curated, proof-bearing guides** seeded live (run `node scripts/seed-concept-hub.mjs` with the Groq + service-role keys to fill all 36 topics).
...
app/
  ...
  api/
    ...
    score/route.js     POST: SERVER-AUTHORITATIVE scoring (signed-in) — dock+grade+scoreFromRubric+Elo+persist; diagnostic batch
...
lib/
  ...
  scoring.js           SUBJECTS/ORDER/bands + clampScore/band + Elo engine (eloUpdate/rankFor/explainRankMove) + 9-axis rubric + weighted scoreFromRubric/contributionBreakdown
...
### Tables
- **`scores`** — `(user_id uuid, subject text check(math|physics|chemistry), score int, weak_concepts text[], comment text, rubric jsonb, updated_at)`, PK `(user_id, subject)`. `rubric` is the per-subject **9-axis reasoning profile** (`{comprehension, principle, justification, strategy, logic, execution_method, computation, verification, communication}`, 0–4 each) that powers the radar — **server-computed only** (see `save_progress_for`), `null` until a first graded result. It's **opaque jsonb** (only `jsonb_typeof='object'` is checked), so the axis-set change needed no migration; `normalizeRubric` coerces any stored legacy 5-axis rubric onto the new axes on read.
...
- `diagnosticSubjectScore(perQuestion)` — combines a subject's **three** diagnostic answers (beginner + intermediate + hard) into a 0–100 **baseline**, weighting each question's reasoning score by its difficulty anchor (beginner 10 / intermediate 50 / advanced 70) — **points proportional to difficulty**. The divisor is floored at the full tier weight (130), so a missing/**skipped** tier counts as 0 (acing only the beginner question → ~8, not 100). Called by `/api/score` (no `blend()` — this is the baseline). Tier-count-agnostic. Exported alongside `DIAGNOSTIC_DIFFICULTIES` (`["beginner","intermediate","advanced"]`) and `DIFFICULTY_LABELS` (Beginner/Intermediate/Hard).
- **Rubric helpers (powering the radar + score breakdown + "what to work on").** `RUBRIC_KEYS`/`RUBRIC_LABELS`/`RUBRIC_SHORT`/`RUBRIC_MAX` — the **9 chain-link axes** in display order (comprehension · principle · justification · strategy · logic · execution_method · computation · verification · communication, scored 0–4) — plus `RUBRIC_WEIGHTS`/`RUBRIC_WEIGHT_SUM` (the per-axis weights, Σ=25; a CI test asserts the weights are keyed exactly to `RUBRIC_KEYS` so a future axis edit can't NaN the score). `normalizeRubric(r)` coerces a model/stored rubric to a complete **integer** 0–4 object AND maps a **legacy 5-axis** rubric onto the new axes (no DB migration — `rubric` is opaque jsonb); `normalizeRubricFloat` is the float-preserving variant used by `blendRubric`/`diagnosticSubjectRubric` for radar resolution. `lowestRubricDimensions(r, n)` — the weakest axes, for "what to work on". All pure, so `/api/score` computes them exactly as the client would.
...
**Vitest**, configured in `vitest.config.js` (node env by default; component tests opt into `jsdom` via a `// @vitest-environment jsdom` docblock; automatic JSX runtime; `@/` alias). Run with `npm test` (CI uses this) or `npm run test:watch`. **454 tests across 33 files**, all passing.
...
| `test/scoring-elo.test.js` | the **item-as-opponent Elo**: `eloExpected`/`eloK` (provisional→stable), `eloUpdate` (**non-additive** loss on a poor at-level outcome, bigger gain for an upset, settled<provisional, difficulty self-calibration, clamps), `defaultDifficultyForBand`, `rankFor` (fixed bands + percentile-cutoff hook), `scoreFromRubric` (transparent, anti-gaming), `explainRankMove`, **+ a deterministic gold-set dock→score→Elo pipeline** |
