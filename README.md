...
- **Reasoning-rich PRACTICE questions.** The *diagnostic* is now a curated bank, but the *practice* questions are still Groq-generated as single-substitution prompts — and **no rubric can measure thinking on an atomic question** (`scripts/eval-grading-ordering.mjs` demonstrates this). Rework `PRACTICE_GEN_SYS` (`lib/groq.js`) to require reasoning surface per band (multi-step / method-choice branch / trap), with optional `reasoningSurface`/`expectedChain` metadata threaded into the grader prompt.
