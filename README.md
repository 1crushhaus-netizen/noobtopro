...
1. **Prove it** — for each subject, three open problems (beginner, intermediate, hard; **9 total**) drawn from a **curated, standardized bank** (`lib/diagnosticBank.js` — no Groq generation, everyone gets the same calibrated questions). You solve them and explain every step — or tap **"I don't know"** (time-locked 10s) to skip one you genuinely don't know, which docks it with **no wasted grading**. The per-subject baseline weights harder questions more (points proportional to difficulty: beginner 10 / intermediate 50 / hard 70), so one pass calibrates where you stand.
...
**What works today:**
- ✅ **3-tier curated diagnostic** (beginner + intermediate + hard per subject, **9 questions**, difficulty-weighted baseline) → per-subject scoring → calibrated practice loop. The diagnostic is a **standardized, hand-curated bank** (`lib/diagnosticBank.js`) served with **zero Groq calls** (no generation, no pool). A **time-locked "I don't know" skip** (10s) docks a question with no grading — saving tokens and frustration. *(Practice questions are still Groq-generated.)*
...
**Nothing is mid-flight** — `main` is fully merged and green. The independent fleet audit (#48), the process-first grading core (#49), and the curated diagnostic bank + skip (#51) are all shipped; the next task is the **reasoning-rich practice questions + optional numeric verifier** (see "Where to start").
...
- **Housekeeping (owner-gated):** drop the now-unused `diagnostic_pool` table + `try_add_diagnostic` RPC in a numbered migration (the diagnostic is served from `lib/diagnosticBank.js`); and run `node scripts/seed-concept-hub.mjs` (Groq + service-role keys) to fill all 36 Concept Hub topics.
...
components/
  ...
  Dashboard.jsx        THE merged "Dashboard" tab (was Profile + Progress): identity + KPIs + by-subject + reasoning radar + leaderboard + recent rank-moves in one no-scroll bento grid; trend charts + answer review open in slide-over drawers; guest gate (blurred preview + sign-in prompt)
...
lib/
  ...
  diagnosticBank.js    CURATED 9-question placement bank (3 subjects × beginner/intermediate/hard) — served with no Groq
...
- **`diagnostic_pool`** *(internal cache — now UNUSED)* — `(id bigint identity, content jsonb, created_at)`. Formerly a read-through pool of generated diagnostics. **Superseded:** the diagnostic is now served from the curated **`lib/diagnosticBank.js`** (zero Groq, fully standardized), so `/api/generate` no longer reads or fills the pool. The table + the `try_add_diagnostic` RPC remain in the schema (drained; reserved/safe to drop in a later migration) but are no longer touched at runtime.
...
- **Practice:** `{ "kind": "practice", "subject": "math"|"physics"|"chemistry", "score": int, "weakConcepts": string[], "recentQuestions"?: string[] }` → `{ subject, topic, topicSlug, targetConcept, reasoningSurface, trap, difficulty, question }`. Subject validated via `ORDER.includes` (prototype-safe); `weakConcepts` capped. `topicSlug` is the LLM-chosen **taxonomy slug** (normalized server-side via `normalizeTopic`) — it keys the per-`(subject, topic, band)` difficulty bucket the rating engine calibrates (§11); the human-readable `topic` is unchanged.
...
- **`{ kind: "practice", subject, question, targetConcept, difficulty, topicSlug, reasoning, [image] }`** — **auth REQUIRED** (`401` without a valid token; `503` if the service-role key is unset). Reads the user's **stored per-axis Glicko-2 state**, the user's **recent attempts** in this subject (the last 20 — for the anti-farm repeat factor), and the **calibrated item difficulty** for the `(subject, topicSlug, band)` bucket — all server-side; the client supplies none of the values the new rating is computed from. Applies the **pre-grade dock**, computes the per-attempt headline as the **transparent weighted mean of the rubric axes** (`scoreFromRubric` — the raw outcome that feeds the rating; the client supplies no score), then runs the **unified Glicko-2 update** (`updateAxisRatings`, §11): each of the 9 axes is rated against the calibrated difficulty, the subject score is the weighted aggregate, and the radar is derived from the same ratings; repeated grinding of a bucket **damps the gain** (anti-farm). Persists score + glicko + rubric + the attempt's `(topic, band)` atomically (with a one-line **`rationale`**, the **typed `errors[]`** + grader `solve`, and the **answer-review detail** via `save_progress_for`'s `p_review`) and calibrates the item-difficulty bucket (`bump_item_difficulty`, non-blocking). Returns `{ reasoningScore, rubric, solve, errors[], finalAnswerMatches, strengths[], improvements[], workedSolution, correctnessNote, socraticHint, microLesson, weakConcepts, newScore, delta, rationale, docked, subjectScore, attempt }`. `strengths`/`improvements` are the post-grade "what you did well / to reach 100" feedback; **`workedSolution` is the full solution, revealed only on a SUBSTANTIVE attempt** (empty on a dock — so "idk" can't extract the answer). A client-supplied `score`/`newScore` is **ignored**.
...
Key flows / handlers:
- `beginDiagnostic()` → `/api/generate` (diagnostic → the curated bank) → **9 questions (3 per subject: beginner + intermediate + hard)** answered one at a time (ordered subject-major, easy→hard; answers keyed by `subject:difficulty`; a grouped progress bar of 3×3 pips + a difficulty label per question). Each question's composer offers a **time-locked (10s) "I don't know" skip** that records an empty answer (docked, no Groq grade) and advances. `submitDiagnostic()` sends all 9 answers to **`/api/score`** (one batched request — the server grades + aggregates) → dashboard. **Signed-in** → the server persists the baseline; **guest** → scores saved to `localStorage`, then the **"save your progress" modal** pops. Practice also has the **"I don't know" skip** (`submitPractice(true)` → empty answer → docked).
...
Components: **SignIn** (provider buttons), **Dashboard** (the merged Profile+Progress tab — identity + stats + radar + leaderboard + reset, with trend-chart/answer-review drawers and a guest gate), **charts** (dependency-free inline SVG primitives), **Leaderboard**, **ReviewList**, **LearnTab** (concept picker + guide sections).

---

## 13. Testing

**Vitest**, configured in `vitest.config.js` (node env by default; component tests opt into `jsdom` via a `// @vitest-environment jsdom` docblock; automatic JSX runtime; `@/` alias). Run with `npm test` (CI uses this) or `npm run test:watch`. **512 tests across 36 files**, all passing.
...
| `test/dashboard.test.jsx` | the merged **Dashboard**: identity, one deduped KPI cluster, by-subject, rank chip, reasoning radar (accessible name) + empty state, "what to work on" → Learn wiring, lazy anonymous leaderboard (own position, no identity leak), confirm-guarded reset, "Not ranked yet" empty state, **trends drawer** (charts hidden until opened, Escape closes, focus moves in), **lazy review drawer** (`loadReviews` only after open), **guest gate** (sign-in prompt + NO auth fetches) |
