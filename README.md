...
> **Shipped & live (all merged):** the **Concept Hub** (PR #29–#31), the **"Prove it" diagnostic** (PR #32), **server-authoritative scoring + reasoning radar** (PR #34/#35, with migrations `0001a`/`0001b` applied), and the **fleet-audit fixes** (PR #36, migration `0002`). Each round was gated by an adversarial pre-merge review.
>
> **Just shipped (the current overhaul, in flight as its own PRs):**
> - **Diagnostic → 6 questions (2/subject, easy+hard)** — PR #37 — to relieve the Groq 429s the 9-question set caused.
> - **Durable, per-account rate limiter** — this branch — `lib/rateLimit.js#checkRateLimit` backed by the Postgres `rate_limit_hit` RPC (migration `0003`, applied to live), keyed by `auth.uid()` (IP for guests), in-memory fallback when the service-role key is unset. `/api/score` adds per-account `:practice`/`:diag` caps.

### The next task — Elo ranking + consistent grading + leaderboard
The owner has **chosen the design** (see the `ranking-grading-direction` memory). Build to it; don't re-litigate:
- **Rating engine: item-as-opponent Elo** with self-calibrating per-question difficulty (the question is the rated opponent; per-subject rating + item difficulty both update each attempt, so difficulty auto-calibrates to the user base). Outcome = graded reasoning quality in `[0,1]`, so a low outcome on an at-level item *loses* rating (non-additive). Evolve `lib/scoring.js`'s existing `blend` toward this; persist a real per-(subject,topic,band) difficulty.
- **5 ranks via FIXED score bands** at launch (reuse the beginner→PhD band pattern). Architect for **percentile-recut tiers** later (the flip happens once there's a user base) but ship fixed bands.
- **Grading: extremely consistent + explainable + anti-gaming.** A deterministic **pre-grader docking gate** (empty/"idk"/off-topic/gibberish → forced low, no LLM call); **anchored rubric exemplars** + rationale-before-score + anti-buzzword/length-normalization clauses in `PRACTICE_GRADE_SYS`/`DIAG_GRADE_SYS`; **temperature 0 (or 3-sample median)** + a **server-side reconciliation** (a `reasoningScore` can't contradict its rubric mean); a **gold-set CI tolerance test**; and persist a one-line **"why your rank moved"** rationale so the Profile shows why every point was gained/docked.
- **Leaderboard on Profile** (per-subject + overall). ⚠️ New, **security-sensitive** surface: showing other users' ranks needs a `SECURITY DEFINER` read returning only display name + rating/rank (no email/per-attempt data), with opt-out — review it like `/api/score` and `/api/admin`.

### Key files (for the next task)
`lib/scoring.js` (the Elo engine + rank bands + reconciliation, kept pure) · `lib/groq.js` (`PRACTICE_GRADE_SYS`/`DIAG_GRADE_SYS` ← anchored exemplars) · `lib/gradeInput.js` or a new pre-grader gate (docking) · `app/api/score/route.js` (apply the engine + gate + reconciliation) · `db/schema.sql` (per-item difficulty table + a leaderboard read RPC) · `components/ProfileTab.jsx` + `ProgressDashboard.jsx` (rank badge + leaderboard).

### Also queued
**Concept Hub: public seed + consistency** — batch-generate a curated baseline catalog across the taxonomy and publish it (`source='curated'`, `visibility='public'`) so every user sees the same vetted hub from day one; plus canonical de-dup of near-identical concepts. (Today the curation-only model means nothing is public until an admin approves.)
