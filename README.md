...
- **Server-side auth enforcement** on the *public* API routes (`/api/generate|grade|learn` are same-origin-gated + rate-limited + RLS-scoped, but not per-user authenticated). The **`/api/admin/*` routes ARE per-user authenticated** — the app's first privileged surface: they verify the caller's Supabase JWT server-side (`lib/adminAuth.js`) against a deny-by-default `ADMIN_EMAILS`/`ADMIN_USER_IDS` allowlist, re-checked on every call, never trusting the client; the admin **approve** action is the only sanctioned path that publishes a guide (human-in-the-loop curation — no *automated* path makes content public). Prompt-injection/abuse signals are logged (non-blocking) to `security_events` for review. Given it can publish content, this surface should get a focused security review before/at each change.
